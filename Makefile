...
preview:
	 docker run $(RM_FLAG) --name cloudbreak-docs-preview -p $(PW_PORT):8000 -v $(PWD):/work sequenceiq/pagebuilder:$(PB_VER) mkdocs serve
...
test:
	docker run $(RM_FLAG) -p 8000:8000 -v $(PWD):/work sequenceiq/pagebuilder:$(PB_VER) mkdocs build

copy-generated-dots:
	cp $(CB_LOCATION)/core/build/diagrams/flow/*.dot docs/diagrams

generate-dots-by-branch:
	docker run $(RM_FLAG) -v $(PWD):/work $(CACHE_OPTION) -e BRANCH=$(CB_BRANCH) -it sequenceiq/pagebuilder:$(PB_VER) generate-dots
